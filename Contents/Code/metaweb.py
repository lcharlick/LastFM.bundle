...
class Session(object):
    ...
    def read(self, *queries, **options):
        ...
        opts = self.getopts("lang", "as_of_time", "escape",
        ...
        outer = self.check(self.fetch(url))
        ...
        for i in range(0, n):
            ...
            self.check(inner)               # Check inner for errors
    ...
    def search(self, query, **options):
        ...
        parameters.
        """
        opts = self.getopts("domain", "type", # Build a dict of search options
        ...
        envelope = self.fetch(url)                        # Fetch response
        self.check(envelope)                              # Check that its OK
	...
	Return the content and type of the content object identified by id.
        ...
        """
        return self.trans(self.contentURL(id))
	...
	Return the content and type of a document blurb.  See blurbURL().
        """
        return self.trans(self.blurbURL(id, **options))
	...
	thumbnail.  See thumbnailURL().
        """
        return self.trans(self.thumbnailURL(id, **options))
	...
	"""
        return self.transURL(id, DOWNLOAD)
	...
	"""
        return self.transURL(id, BLURB, ["maxlength", "break_paragraphs"],
                              options)
	...
	"""
        return self.transURL(id, THUMBNAIL, ["maxwidth","maxheight"], options)
    ...
    def getopts(self, *keys, **local_options):
        o = {}
        for k in keys:
    ...
    def http(self, url, headers={}, body=None):
    ...
    def dumpjson(self, o, pretty=False):
        if pretty:
    ...
    def fetch(self, url, headers={}, body=None):
        # Fetch the URL contents
        return JSON.ObjectFromURL(url, headers=headers, cacheTime=CACHE_1WEEK)
        
        (status, headers, body) = self.http(url, headers, body);
    ...
    # cookie management and error handling the way the fetch function does.
    # Unlike other Metaweb services, trans does not normally return a JSON 
    # object, so we cannot just use fetch here.
    def trans(self, url):
        (status,headers,body) = self.http(url)   # Fetch url content
        if (status == 200):                       # If successful
            return body,headers['content-type']   # Return content and type
        else:                                     # HTTP status other than 200
            errobj = self.parsejson(body)        # Parse the body
    ...
    def transURL(self, id, service, option_keys=[], options={}):
        url = "http://" + self.host + service + id    # Base URL.
        opts = self.dumpgetopts(*option_keys, **options) # Get request options.

...
#######################################################################
def TagTopArtists(tagName, URLEncode = True):
  if URLEncode:
    query = String.URLEncode(query)
  else:
    query = query.replace(' ','+')
  url = TAG_TOP_ARTISTS % String.URLEncode(tagName)
  return TopArtists(url)

#######################################################################
def TagTopAlbums(tagName, URLEncode = True):
  if URLEncode:
    query = String.URLEncode(query)
  else:
    query = query.replace(' ','+')
  url = TAG_TOP_ALBUMS % String.URLEncode(tagName)
  return TopAlbums(url)

#######################################################################
def TagTopTracks(tagName, URLEncode = True):
  if URLEncode:
    query = String.URLEncode(query)
  else:
    query = query.replace(' ','+')
  url = TAG_TOP_TRACKS % String.URLEncode(tagName)
...
def SimilarTags(tag):
    tags = []
    url = TAG_SIMILAR_TAG % (String.Quote(tag[0].encode('utf-8'), True))
...
def AlbumTrackList(artistName, albumName):
    tracks = []
    albumInfo = XML.ElementFromURL( ALBUM_INFO % (String.Quote(artistName.encode('utf-8'), True), String.Quote(albumName.encode('utf-8'), True)))
    ...
    for trackElement in XML.ElementFromURL(playListUrl).xpath('//ns:playlist/ns:trackList/ns:track', namespaces=PLAYLIST_NS):
        ...
        #trackInfo = XML.ElementFromURL(TRACK_INFO % (String.Quote(artistName.encode('utf-8'), True), String.Quote(trackName.encode('utf-8'), True)))
        #image = None
        ...
        track = (trackName, artist, None, trackUrl, False)
        tracks.append(track)
    return tracks
...
def LovedTracks(user, page):
    ...
    # LovedTracks does not return streamable there must use detailed meta-data
        trackInfo =  XML.ElementFromURL(TRACK_INFO % (String.Quote(artistName.encode('utf-8'), True), String.Quote(name.encode('utf-8'), True)))
        streamable = False
        infoElements = trackInfo.xpath('/lfm/track/streamable')
        if len(infoElements) > 0:
            streamable = infoElements[0].text == "1"
...
def SimilarArtists(artistName):
        artists = []
        url = ARTIST_SIMILAR % String.Quote(artistName.encode('utf-8'), True)
...
def ArtistTracks(artistName):
        tracks = []
        url = ARTIST_TRACKS % String.Quote(artistName.encode('utf-8'), True)
        for trackElement in XML.ElementFromURL(url).xpath('/lfm/toptracks/track'):
...
def ArtistVideos(artistName, page):
    videos = []
    url = ARTIST_VIDEOS % (String.Quote(artistName.encode('utf-8')), page)
...
def UserDetails(user):
    url = USER_INFO % String.Quote(user.encode('utf-8'))
    realName = ""
...
##########################################
def IsAuthenticated():
    if Dict[AUTH_KEY] == None:
        Authenticate()
    return Dict[AUTH_KEY] != None

def IsSubscriber():
    if Dict[SUBSCRIBE] == None:
        Authenticate()
    return Dict[SUBSCRIBE] == '1'
